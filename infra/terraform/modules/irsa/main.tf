# ------------------------------------------------------------
# KANIKO ROLE — used by Jenkins agents to push to ECR
# ServiceAccount: jenkins-agents/jenkins
# ------------------------------------------------------------
resource "aws_iam_role" "kaniko" {
  name = "${var.project_name}-kaniko-role"
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${var.oidc_issuer_url}:sub" = "system:serviceaccount:jenkins-agents:jenkins"
...
# Kaniko ECR push policy — scoped to vjcloudbank/* repos only
...
# LBC policy — from official AWS docs
